...
function setupWebSocket(server: any) {
  ...
  io.on('connection', (socket: any) => {
    ...
    // Helper function to safely access unread counts
    const getUnreadCount = (match: any, userId: string): number => {
      if (!match.unreadCounts) return 0

      // Use type assertion to handle both Map and object
    ...
    // Helper function to set unread count
    ...
    socket.on('send-message', async (data: any) => {
      ...
      try {
        ...
        // Type assertion for populated sender
        ...
        // Increment unread count for other user using helper
        const otherUser = match.users.find(
          (u: any) => u.toString() !== socket.userId
        )
        if (otherUser) {
