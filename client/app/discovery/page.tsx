...
const DiscoveryPage: React.FC = () => {
  ...
  // Check localStorage for auth status on mount (FAST CHECK)
  const [localAuthChecked, setLocalAuthChecked] = useState(false)

  useEffect(() => {
    // First check: Look for token in cookies (fastest check)
    ...
    // Second check: Look for auth data in localStorage
    ...
    console.log('🔐 Fast auth check starting...')

    // If no token in cookies and no localStorage auth, we can redirect immediately
    const hasTokenInCookies = checkTokenInCookies()
    const localStorageAuth = checkLocalStorage()

    console.log('🍪 Token in cookies:', hasTokenInCookies)
    console.log('📦 Auth in localStorage:', !!localStorageAuth)

    // If we have neither cookies nor localStorage, redirect immediately
    if (!hasTokenInCookies && !localStorageAuth) {
      console.log('🚫 No auth data found, redirecting immediately...')
      setIsRedirecting(true)
      setTimeout(() => {
        router.replace('/login')
      }, 100)
    ...
    // Mark local auth check as complete
    setLocalAuthChecked(true)

    // If we have localStorage auth but Redux doesn't have user yet, dispatch check
    if (localStorageAuth && !authUser && !checkingAuth && !authLoading) {
      console.log('🔄 Found auth in localStorage, verifying with server...')
      dispatch(checkAuthRequest())
    }
  }, [dispatch, router, authUser, checkingAuth, authLoading])

  // Main authentication effect - runs after local check
  useEffect(() => {
    // Don't run if local auth check hasn't completed or we're already redirecting
    if (!localAuthChecked || isRedirecting) {
      return
    }
    ...
    // If we're not already checking and not loading, start the server check
    if (!checkingAuth && !authLoading) {
  ...
  // Handle auth check results and redirects
  useEffect(() => {
    // Don't process if local check hasn't completed
    if (!localAuthChecked) {
      return
    }

    console.log('🔄 Processing auth check results:', {
      checkingAuth,
      authLoading,
      hasUser: !!authUser,
      isRedirecting,
    ...
    // If we have a user, proceed with app
    if (authUser) {
      console.log('✅ User authenticated, proceeding with app...')

      // Store auth data in localStorage for faster future checks
      try {
        ...
        console.log('⚠️ Could not save auth to localStorage')
      }
    ...
    if (!authUser && !isRedirecting) {
      ...
      // Short timeout to show redirecting message briefly
      const redirectTimer = setTimeout(() => {
        console.log('➡️ Redirecting to login page')
        router.replace('/login')
      }, 300)
  ...
  useEffect(() => {
    if (authError && authError.includes('logout') && !isRedirecting) {
      try {
        localStorage.removeItem('auth')
      } catch (e) {
        // Ignore
  ...
  // Show immediate redirect if no auth data found
  if (isRedirecting && !localAuthChecked) {
    return <LoadingScreen message='Redirecting to login...' />
  }
  ...
  // Show loading if redirecting after auth check
  if (isRedirecting) {
    return <LoadingScreen message='Redirecting to login...' />
  }
