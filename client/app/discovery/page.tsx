...
interface UserCardProps {
  ...
  onPhotoSwipe: (e: React.MouseEvent) => void
  onPrevPhoto: () => void
  onNextPhoto: () => void
  onImageClick: () => void
  showLikedOnly: boolean
...
const DiscoveryPage: React.FC = () => {
  ...
  const [retryCount, setRetryCount] = useState(0) // ADDED: Track retry attempts
  const MAX_RETRIES = 3 // ADDED: Max retry attempts
  ...
  const { totalUnread, loadingUnread, resetUnread } = useUnreadCount()

  // ... rest of your existing code ...

  // Update handleViewMatches to reset unread count
  const handleViewMatches = useCallback(() => {
    console.log('📨 Going to matches, resetting unread count')
    resetUnread()
    router.push('/matches')
  }, [resetUnread, router])
  ...
  // Update the Header component to show the badge
  const Header = ({
    onViewProfile,
    onViewMatches,
    disabled,
    ...
    <header className='bg-white shadow-sm py-4 px-6'>
      <div className='max-w-2xl mx-auto flex justify-between items-center'>
        ...
        <div className='flex items-center gap-2'>
          {/* Match button with unread badge */}
          <div className='relative'>
            <Button
              title='💌'
              onClick={onViewMatches}
              disabled={disabled || loadingUnread}
              btnStyle='w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors disabled:opacity-50 p-0 min-w-0 text-lg'
            />

            {/* Unread message badge - Only show when not loading and has unread */}
            {!loadingUnread && totalUnread > 0 && (
            ...
            {/* Loading indicator */}
            {loadingUnread && (
              <div className='absolute -top-1 -right-1 bg-gray-400 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center shadow-lg'>
  ...
  // Optimized geolocation fetching
  ...
  // Optimized geolocation function
  ...
  // Refresh handler - with retry reset
  ...
  useEffect(() => {
    console.log('🔄 Checking if should fetch data:', {
      authUser: !!authUser,
    ...
    // Only fetch if we have BOTH authUser AND profileUser
  ...
  // Shuffle users when recommendations are loaded - WITH RETRY LOGIC
  useEffect(() => {
    console.log(
      'recommendedUsers-------',
    ...
    // Only process if we have recommendations data
    if (recommendedUsers.length > 0 && isInitialLoad) {
      ...
      setTimeout(() => {
        ...
        console.log('✅ Users loaded and initial load completed')
      }, 0)
    } else if (
      ...
      // CRITICAL FIX: Don't immediately show empty state, try to refetch
      console.log(
        '📭 No recommendations received, checking if we should retry...'
      )

      // If we have a profile and haven't exceeded retry limit, try again
      if (profileUser && retryCount < MAX_RETRIES) {
        console.log(
        ...
        setTimeout(() => {
          ...
          setHasFetched(false)
          // Force a small delay before refetching
          setTimeout(() => {
            getLocationAndFetch()
          }, 1000 * retryCount) // Exponential backoff
        }, 1000)
      } else if (retryCount >= MAX_RETRIES) {
        // After max retries, show empty state
        console.log('⏹️ Max retries reached, showing empty state')
        setTimeout(() => {
          setShuffledUsers([])
          setIsInitialLoad(false)
          console.log('✅ Empty state set after max retries')
        }, 0)
      } else {
        // No profile yet, just wait
        console.log('⏳ Waiting for profile before showing empty state')
      }
    }
  }, [
    recommendedUsers,
  ...
  // Safety timeout to ensure isInitialLoad gets reset
  useEffect(() => {
    if (isInitialLoad) {
      const timeoutId = setTimeout(() => {
        console.log('⏰ Safety timeout - resetting initial load after delay')
        setIsInitialLoad(false)
  ...
  // FIXED: Show loading while we're retrying
  const showDataLoading =
    (authUser && !profileUser && profileLoading) ||
    (isInitialLoad && recommendationsLoading) ||
    (isInitialLoad && loadingSwipeHistory) ||
    (isInitialLoad && retryCount > 0 && retryCount < MAX_RETRIES) // Show loading while retrying

  if (showDataLoading) {
    const retryMessage =
      retryCount > 0 ? ` (retrying ${retryCount}/${MAX_RETRIES})` : ''
    return <LoadingScreen message={`Loading discovery${retryMessage}...`} />
  }
  ...
  // Only show empty state if we're not in initial load
  if (!isInitialLoad) {
  ...
  // If we have no currentUser at this point, check if we should show users anyway
  if (!currentUser) {
    // If we have shuffled users but no displayUsers due to filtering, show first user
    if (shuffledUsers.length > 0 && shuffledUsers[0]) {
      const fallbackUser = shuffledUsers[0]
      const hasMultiplePhotos =
        fallbackUser.photos && fallbackUser.photos.length > 1
      const isLiked = localLikedUsers.has(fallbackUser._id)
